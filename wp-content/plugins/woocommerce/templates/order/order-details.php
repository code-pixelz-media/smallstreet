	...
	<table class="woocommerce-table woocommerce-table--order-details shop_table order_details">
		...
		<tbody>
			...
			foreach ($order_items as $item_id => $item) {
				...
				// Get XP for this specific item/order
				$xp_earned = 0;
				...
				if (!empty($current_user_buyer_details) && is_array($current_user_buyer_details)) {
					foreach ($current_user_buyer_details as $detail) {
						if (isset($detail['order_id']) && intval($detail['order_id']) == $order->get_id()) {
							$xp_earned = isset($detail['xp_awarded']) ? intval($detail['xp_awarded']) : 0;
							break;
						}
					}
				}
		...
		<tfoot>
			...
			if (!empty($membership_name)): ?>
				<tr>
					...
					<td><strong><?php echo number_format(1000000); ?> XP</strong></td>
				</tr>
			<?php endif; ?>
		</tfoot>
	</table>
